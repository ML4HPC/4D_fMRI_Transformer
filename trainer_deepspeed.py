...
class Trainer():
    ...
    def __init__(self,sets,**kwargs):
        ...
        pths = self.find_pth(self.writer.experiment_folder)
        if len(pths) == 0:
            self.st_epoch = 0
            ...
            if self.deepspeed:
                _, client_sd = self.model_engine.load_checkpoint(self.writer.experiment_folder)
                self.st_epoch = client_sd['epoch']
                self.best_loss = client_sd['loss_value']
                self.best_accuracy = client_sd['accuracy']
...
#         torch.backends.cudnn.benchmark = True   
    ...
    def train_epoch(self,epoch):       
        ...
        for batch_idx, input_dict in enumerate(tqdm(self.train_loader,position=0,leave=True)): 
            ...
            if self.deepspeed:
                loss_dict, loss = self.forward_pass_ds(input_dict)
    ...
    def eval_epoch(self,set):
        ...
        with torch.no_grad():
            ...
            for batch_idx, input_dict in enumerate(tqdm(loader, position=0, leave=True)):
                # start_time = time.time()
                with autocast():
                    loss_dict, _ = self.forward_pass_ds(input_dict)
                # end_time = time.time()
    ...
    def forward_pass_ds(self,input_dict):
        input_dict = {k:(v.to(self.gpu) if (self.cuda and torch.is_tensor(v)) else v) for k,v in input_dict.items()}
        #print('shape of input dict is :', input_dict['fmri_sequence'].size())
        output_dict = self.model_engine(input_dict['fmri_sequence'].half())
        torch.cuda.nvtx.range_push("aggregate_losses")
        loss_dict, loss = self.aggregate_losses(input_dict, output_dict)
        torch.cuda.nvtx.range_pop()
        if self.task == 'fine_tune':
    ...
    def compute_perceptual(self,input_dict,output_dict):
        fmri_sequence = input_dict['fmri_sequence'][:,0].unsqueeze(1)
        #deepspeed
        perceptual_loss = self.perceptual_loss_func(output_dict['reconstructed_fmri_sequence'].float(),fmri_sequence)
