...
class DenseNet(nn.Module):
    ...
    def _make_fclayers(self):
        ...
        #for cat_label in self.cat_target:
            self.out_dim = 2 #len(self.subject_data[cat_label].value_counts())                        
            classifiers.append(nn.Sequential(nn.Linear(self.num_features, self.out_dim)))

        for num_label in self.num_target:
            classifiers.append(nn.Sequential(nn.Linear(self.num_features, 1)))

        return nn.ModuleList(classifiers)
